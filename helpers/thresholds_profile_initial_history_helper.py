...
def create_thresholds_profiles_initial_history():
    for schema in schemas:
        with schema_context(schema):
            ...
            response = requests.get(
                settings.WEBAPI_THRESHOLDS, headers=headers
            )

            data = response.json()['data']

            profiles = poem_models.ThresholdsProfiles.objects.all()
...
def main():
    create_thresholds_profiles_initial_history()
