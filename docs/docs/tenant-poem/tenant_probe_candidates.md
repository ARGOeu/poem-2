...
Probe candidates page is only accessible to POEM admins from [Administration](administration/tenant_administration.md) page. The layout is shown in the figure below. One can see probe candidate names, descriptions, date of creation and statuses. There are five available statuses: `submitted`, `processing`, `testing`, `deployed`, and `rejected`.
